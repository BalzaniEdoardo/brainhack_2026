...
def load_trials(dataset_num:int, root_folder: str | Path | None = None) -> nap.IntervalSet:
    ...
    trials = nap.IntervalSet(
        trials["trial_start"].to_numpy(),
        trials["trial_end"].to_numpy(),
        metadata=meta
    )
    return trials
...
def load_lfp(dataset_num: int, root_folder: str | Path | None = None, fs_hz=500, electrode_spacing_um=20) -> nap.TsdFrame:
    ...
    lfps = nap.TsdFrame(time, lfps, metadata=dict(depth_um=depth_um, brain_area=brain_area))
    return lfps
